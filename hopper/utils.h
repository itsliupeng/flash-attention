...
__forceinline__ __device__ uint64_t resolve_page_slice_offset(const int* block_table, const int n_block, const int kBlockN, const int page_size, const int page_stride, const int row_stride) {
    const uint64_t block_offset = n_block * kBlockN;
    const uint64_t virtual_page_idx = block_offset / page_size;
    const uint64_t page_offset = block_offset % page_size;
    const uint64_t global_offset = ((uint64_t) block_table[virtual_page_idx]) * ((uint64_t) page_stride) + ((uint64_t) page_offset) * ((uint64_t) row_stride);
