...
constexpr static int Stages_fp8 = 2;
...
void run_flash_fwd(Flash_fwd_params &params, cudaStream_t stream) {
    ...
    // free 
    if ((params.block_table != nullptr) && (params.tensormaps != nullptr)) {
        cudaFreeAsync(params.tensormaps, stream);
    }
}
...
void run_mha_fwd_hdim64(Flash_fwd_params &params, cudaStream_t stream) {
    ...
    BOOL_SWITCH(params.is_causal, Is_causal, [&] {
        SEQLEN_SWITCH(params.cu_seqlens_q, Seqlen_traits, [&] {
            run_flash_fwd<
                Flash_fwd_kernel_traits<Headdim, 192, 128, 16, Stages, false, 1, T>, 
    ...
    BOOL_SWITCH(params.is_causal, Is_causal, [&] {
        SEQLEN_SWITCH(params.cu_seqlens_q, Seqlen_traits, [&] {
            ...
            BOOL_SWITCH(cutlass::ceil_div(params.seqlen_q, 128) % 2 == 0 && !Is_causal && !Seqlen_traits::kUseVarSeqLen, UseCluster, [&] {
                run_flash_fwd<
                    Flash_fwd_kernel_traits<Headdim, 128, Is_causal ? 128 : 176, 12, Stages, false, UseCluster ? 2 : 1, T>, 
                    Is_causal, Seqlen_traits
                >(params, stream);
            });
        });
...
void run_mha_fwd_hdim256(Flash_fwd_params &params, cudaStream_t stream) {
    ...
    BOOL_SWITCH(params.is_causal, Is_causal, [&] {
        SEQLEN_SWITCH(params.cu_seqlens_q, Seqlen_traits, [&] {
            ...
            BOOL_SWITCH(cutlass::ceil_div(params.seqlen_q, 128) % 2 == 0 && !Is_causal && !Seqlen_traits::kUseVarSeqLen, UseCluster, [&] {
                run_flash_fwd<
                    Flash_fwd_kernel_traits<Headdim, 128, 80, 12, Stages, false, UseCluster ? 2 : 1, T>, 
                    Is_causal, Seqlen_traits
                >(params, stream);
            });
        });
...
void run_mha_fwd_hdim256_fp8(Flash_fwd_params &params, cudaStream_t stream) {
    ...
    constexpr static int kNWarps = 12;
    constexpr static int kStages = Stages_fp8;    
    BOOL_SWITCH(params.is_causal, Is_causal, [&] {
        SEQLEN_SWITCH(params.cu_seqlens_q, Seqlen_traits, [&] {
            // Only use Cluster if number of tiles along seqlen_q is even
...
template<typename T>
void run_mha_fwd_hdim512_fp8(Flash_fwd_params &params, cudaStream_t stream) {
...
template<typename T>
void run_mha_fwd_hdim576_fp8(Flash_fwd_params &params, cudaStream_t stream) {
